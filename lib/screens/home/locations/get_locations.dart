...
enum GeofenceEvented { init, enter, exit }
...
class Geofenced {
  ...
  static Stream<GeofenceEvented>? _geostream;

  /// [_controller] is Stream controller for geofence event stream
  static StreamController<GeofenceEvented> _controller =
      StreamController<GeofenceEvented>();
  ...
  static Stream<GeofenceEvented>? getGeofenceStreamed() {
    return _geostream;
  }
  ...
  static startGeofenceService(
    ...
    if (_positionStream == null) {
      ...
      _controller.add(GeofenceEvented.init);
    }
  }
  ...
  static _checkGeofence(double distanceInMeters, double radiusInMeter) {
    if (distanceInMeters <= radiusInMeter) {
      _controller.add(GeofenceEvented.enter);
    } else {
      _controller.add(GeofenceEvented.exit);
    }
  }
